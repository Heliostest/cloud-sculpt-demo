...
> 状态：**实现完成，WebGPU 后测待补**（2026-08-09）。阶段 9 的 8-body 场景暴露出 130.359 ms 的高成本；本阶段完成无损高度粗裁剪、静态天气采样复用、实际 body count 提前结束和低频光照 probe。自动本地浏览器访问被环境安全策略拒绝，因此不虚构 after GPU 数值。记录见 `docs/evidence/genus-morphology/stage10-performance/`。

### 16.1 任务

- [x] 固定单体、8-body 与 8-body simpleMode 同机基线；
- [x] 把 CPU 已知的实际 volume body 数量传给 WGSL，避免遍历空槽；
- [x] 在 body weather、局部坐标和 genus dispatcher 前做严格 base/top 高度拒绝；
- [x] 静态 body 复用 world-space weather sample，移动/形变 body 保持 transported weather；
- [x] 光照密度 probe 使用既有 simple morphology 近似；
- [x] 增加 `bodyCount=1..8` 压力场景 URL 与运行时 dataset；
- [x] 增加结构回归测试，锁定候选数与 cheap-light 语义；
- [ ] 在允许访问本地 WebGPU 页的环境补采 1/2/4/8 after timestamp-query 曲线。

### 16.2 验收

- 优化不得改变 view-ray 的最终 detail density；
- 严格高度拒绝必须与 evaluator 原有 base/top 边界一致；
- motion、morph 与 lifecycle 语义保持；
- 8-body 三个代表高度的昂贵候选数从固定 8 个降为 4/3/3；
- 全部测试、构建和 WGSL 运行时验证通过；最后一项在 after WebGPU 复测前保持未完成。
...
- [ ] 阶段 9：十云属最终校准。
- [ ] 阶段 10：多云体性能与交付硬化（实现完成，WebGPU 后测待补）。

## 21. 下一步

下一步只补阶段 10 的实测闭环，或在用户明确恢复后重新规划阶段 6：

1. 在允许本地 WebGPU 自动化的环境采集 `bodyCount=1/2/4/8` after 曲线；
2. 与阶段 9 的 1-body 和 8-body 基线同机比较，并记录中位数与样本数；
3. 若仍超预算，再评估每射线 body interval mask 或低分辨率/时域方案，不降低固定证据质量；
4. Ci 继续保持“未完成、暂时跳过”，不作为性能阶段的隐藏工作。
