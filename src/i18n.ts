...
const FOLDERS: Record<string, { label: Copy; tip: Copy }> = {
  ...
  layers: { label: copy('Generic 3D Layers (not HP High)', '通用 3D 云层（不含 HP 中高云）'), tip: copy('Three general cloud volumes. Use these for genus and altitude composition.', '三个通用云体积层，用于组合云属和高度。') },
...
const PARAMETERS: Record<string, ParameterCopy> = {
  ...
  highCloudEnabled: parameter('Enable High Cloud', '启用中高云', 'Enables the independent altocumulus/altostratus layer.', '启用独立的高积云 / 高层云层。'),
  highCloudGenus: parameter('High-Cloud Genus', '中高云云属', 'Chooses altocumulus or altostratus shaping.', '选择高积云或高层云塑形。'),
  ...
  highBaseKm: parameter('Base Altitude (km)', '云底高度（km）', 'Bottom altitude of the high-cloud layer.', '中高云层的底部海拔。'),
  highTopKm: parameter('Top Altitude (km)', '云顶高度（km）', 'Top altitude of the high-cloud layer.', '中高云层的顶部海拔。'),
  ...
  highDensityMultiplier: parameter('Density Multiplier', '密度倍率', 'Final high-cloud density scale.', '中高云的最终密度倍率。'),
  ...
  highWispStrength: parameter('Wisp Strength', '丝缕强度', 'Blend strength of wispy high-cloud detail.', '中高云丝缕细节的混合强度。'),
